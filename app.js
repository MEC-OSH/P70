...
function renderTable(filter=""){tbody.innerHTML="";data.filter(x=>x.name.toLowerCase().includes(filter.toLowerCase())).forEach((x,i)=>{const tr=document.createElement("tr");tr.innerHTML=`<td>${data.indexOf(x)+1}</td><td>${x.name}</td><td>${x.ua}</td><td>${x.uc}</td>`;tbody.appendChild(tr)})}
renderTable();document.getElementById("categorySearch").addEventListener("input",e=>renderTable(e.target.value));
const catSelect=document.getElementById("reportCategory");data.forEach(x=>catSelect.add(new Option(x.name,x.name)));

function drawChart(){
 const c=document.getElementById("trendChart"),ctx=c.getContext("2d"),dpr=window.devicePixelRatio||1,w=1100,h=1450;c.width=w*dpr;c.height=h*dpr;ctx.scale(dpr,dpr);ctx.clearRect(0,0,w,h);
 const left=360,top=50,row=46,max=210,scale=620/max;ctx.font="14px Arial";
 data.forEach((x,i)=>{const y=top+i*row;ctx.fillStyle="#111";ctx.fillText(`${i+1}. ${x.name}`,10,y+20);ctx.fillStyle="#08a83e";ctx.fillRect(left,y,x.ua*scale,14);ctx.fillStyle="#111";ctx.fillText(x.ua,left+x.ua*scale+8,y+12);ctx.fillStyle="#e31b23";ctx.fillRect(left,y+19,x.uc*scale,14);ctx.fillStyle="#111";ctx.fillText(x.uc,left+x.uc*scale+8,y+31)});
 ctx.fillStyle="#08a83e";ctx.fillRect(left,10,18,12);ctx.fillStyle="#111";ctx.fillText("Unsafe Acts",left+25,21);ctx.fillStyle="#e31b23";ctx.fillRect(left+150,10,18,12);ctx.fillStyle="#111";ctx.fillText("Unsafe Conditions",left+175,21)
}
drawChart();
